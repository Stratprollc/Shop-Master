...
Congratulations on your POS system! Version v4.2.4 is ready for release. To make it live on **pos.sellerscampus.com** using Hostinger, follow these steps:
...
4. **Application Configuration**:
   ...
   - **Entry File**: Point this to `dist/server.cjs` (This is the pre-compiled file generated by `npm run build` which bundles your `server.ts` perfectly).
   - **Port Settings (Automatic)**: **You do not need to find or configure a "Port" field in Hostinger!** Hostinger assigns a dynamic port automatically in the background and passes it to our application through `process.env.PORT`. Our compiled code is designed to automatically detect this and connect.
5. **Environment Variables**:
   - Go to the **Environment Variables** tab inside the Hostinger Node.js manager interface.
